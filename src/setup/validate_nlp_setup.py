...
def validate_beautifulsoup() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "detail": "BeautifulSoup4 available"
        }
    except ImportError:
        return {
            "status": "❌",
            "detail": "BeautifulSoup4 not found. Run: uv sync"
        }
...
def validate_lxml() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "✅",
            "detail": "lxml available (fast C-based parser)"
        }
    except ImportError:
        return {
            "status": "⚠️",
...
def validate_tiktoken() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "✅",
            "detail": f"tiktoken working (test: {tokens} tokens)"
        }
    except Exception as e:
        return {
            "status": "❌",
            "detail": f"tiktoken error: {str(e)}"
        }
...
def validate_pydantic() -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "❌",
            "detail": "Pydantic not found. Run: uv sync"
        }
...
def main() -> int:
    ...
    print(f"\n{BOLD}{'='*70}{RESET}")
    critical_ok = (
        py_result["status"] == "✅" and
        sp_result["status"] == "✅" and
        pd_result["status"] == "✅" and
        tk_result["status"] == "✅"
    )
    ...
    if critical_ok:
        ...
        if lx_result["status"] == "⚠️":
            print(f"  • Install lxml: check system dependencies above")
        print(f"{BOLD}{'='*70}{RESET}\n")
        return 0
    else:
